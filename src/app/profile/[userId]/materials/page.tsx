...
import { MaterialCard } from "@/components/profile/material-card-client";

export default async function UserMaterialsPage({ params }: { params: Promise<{ userId: string }> }) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 p-6 lg:p-10">
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{user.name}'s Materials</h1>
                        <p className="text-slate-500 dark:text-gray-400">View complete catalog</p>
                    </div>
                </div>
                ...
                {user.materials && user.materials.length > 0 ? (
                    ...
                    <div className="text-center py-20">
                        ...
                        <p className="text-slate-500 dark:text-gray-400 text-lg">No materials found in catalog.</p>
                    </div>
                )}
            </div>
        </div>
